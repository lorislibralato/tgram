...
fn main() {
    ...
    File::open("data/schema/api.tl").unwrap().read_to_string(&mut api_schema_text).unwrap();
    let api_schema = schema(&api_schema_text).unwrap().1;

    let mut mtproto_schema_text = String::new();
    File::open("data/schema/mtproto.tl").unwrap().read_to_string(&mut mtproto_schema_text).unwrap();
